...
export default function Modal(props) {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { style: { background: '#111', border: '1px solid #252525', borderRadius: 16, padding: 24, width: '100%', maxWidth: 420, maxHeight: '90vh', overflowY: 'auto' } },
      React.createElement('h2', { style: { fontSize: 14, fontWeight: 700, color: '#fff', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 20 } }, tile ? 'Modifier' : 'Nouveau lien'),
      ...
      React.createElement('div', { style: { marginBottom: 14 } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: 8 } },
          ...
          React.createElement('button', { onClick: function() { setFav(function(v) { return !v }) }, title: 'Favori', style: { width: 44, background: fav ? '#1a1200' : '#161616', border: fav ? '1px solid #b45309' : '1px solid #2a2a2a', borderRadius: 8, fontSize: 20, cursor: 'pointer' } }, '⭐')
        )
      ),
      ...
      React.createElement('div', { style: { marginBottom: 14 } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: 8 } },
          React.createElement('button', { onClick: function() { setOpenMode('tab') }, style: { flex: 1, padding: 9, borderRadius: 8, border: openMode === 'tab' ? '1px solid #CC0000' : '1px solid #2a2a2a', background: openMode === 'tab' ? '#1a0000' : '#0a0a0a', color: openMode === 'tab' ? '#CC0000' : '#666', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' } }, 'Nouvel onglet'),
          React.createElement('button', { onClick: function() { setOpenMode('iframe') }, style: { flex: 1, padding: 9, borderRadius: 8, border: openMode === 'iframe' ? '1px solid #1a6fc4' : '1px solid #2a2a2a', background: openMode === 'iframe' ? '#001a2a' : '#0a0a0a', color: openMode === 'iframe' ? '#1a6fc4' : '#666', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' } }, 'Dans la page')
        )
      ),
        ...
        React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 } },
          allCats.map(function(c) {
            return React.createElement('button', {
              key: c, onClick: function() { setCat(cat === c ? '' : c) },
