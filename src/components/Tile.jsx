...
export default function Tile(props) {
  ...
  if (!editMode && tile.url && tile.url !== '#' && mode !== 'iframe') {
  ...
  if (showIframe) {
    return React.createElement('div', { style: { position: 'fixed', inset: 0, background: '#080808', zIndex: 300, display: 'flex', flexDirection: 'column' } },
      React.createElement('div', { style: { padding: '10px 16px', background: '#0d0d0d', borderBottom: '1px solid #1a1a1a', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 } },
        React.createElement('button', { onClick: function() { setShowIframe(false) }, style: { background: 'transparent', border: '1px solid #2a2a2a', borderRadius: 6, color: '#aaa', fontSize: 13, padding: '6px 14px', cursor: 'pointer', fontFamily: 'inherit' } }, '<- Retour'),
        ...
        React.createElement('a', { href: tile.url, target: '_blank', rel: 'noopener noreferrer', style: { marginLeft: 'auto', fontSize: 11, color: '#555', textDecoration: 'none', border: '1px solid #222', borderRadius: 5, padding: '4px 10px' } }, 'Nouvel onglet')
      ),
