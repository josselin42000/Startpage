...
export default function Tile(props) {
  ...
  var linkProps = {
    ...
    style: {
      ...
      borderRadius: 14, padding: '22px 14px 16px', cursor: 'pointer',
      textAlign: 'center', display: 'flex', flexDirection: 'column',
      alignItems: 'center', gap: 11, textDecoration: 'none',
      position: 'relative', userSelect: 'none', animation: anim,
    }
  }

  if (tile.url && tile.url !== '#') {
    linkProps.href = tile.url
  ...
  return React.createElement('a', linkProps,
    ...
    React.createElement('div', {
      style: {
        ...
        flexShrink: 0, border: '1px solid rgba(128,128,128,0.2)', background: 'rgba(128,128,128,0.1)',
      }
    },
      tile.logo
        ...
        : isEmoji
          ...
          : React.createElement('span', { style: { fontSize: 14, fontWeight: 700, color: tile.color } }, getInitials(tile.name))
    ),
    React.createElement('div', {
      style: { fontSize: 12, fontWeight: 600, color: '#ddd', lineHeight: 1.3, wordBreak: 'break-word' }
    }, tile.name),
    tile.cat ? React.createElement('div', {
      style: { fontSize: 9, color: '#666', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '2px 8px', border: '1px solid #2a2a2a', borderRadius: 20 }
    }, tile.cat) : null
  )
}
